...
class Mav_Nmpc_Tracker:
    def __init__(self, mpc_form_param, tracking_mode, yaw_command_mode):
        ...
        self.thrust_bias_ = self.mpc_form_param_.thrust_bias
    ...
    def real_thrust_to_thrust_factor(self, real_thrust):
        temp = real_thrust - self.thrust_bias_
        if temp < 0:
            temp = 0
        return math.sqrt(temp / self.thrust_scale_)
    ...
    def calculate_roll_pitch_yawrate_thrust_cmd(self):
        ...
        if self.mpc_success_ is True:
            ...
            thrust_cmd = self.real_thrust_to_thrust_factor(self.mpc_u_now_[2]*self.mass_)
        else:
            rospy.logwarn('MPC failure! Default commands sent.')
            roll_cmd = 0.0
            pitch_cmd = 0.0
            thrust_cmd = self.real_thrust_to_thrust_factor(1.0*g*self.mass_)
        ...
        thrust_min = self.real_thrust_to_thrust_factor(self.mpc_form_param_.thrust_min*self.mass_)
        thrust_max = self.real_thrust_to_thrust_factor(self.mpc_form_param_.thrust_max*self.mass_)
...
def nmpc_tracker_control():
    ...
    mpc_form_param.thrust_bias = rospy.get_param("~thrust_bias")
